...
export default function TimeMenu({ id }: { id: string }) {
  ...
  const [movieTime, setMovieTime] = useState<TimeMenu[] | null>(null);
  ...
  const handleButtonClick = (selectedTime: string) => {
    const scheduleData = useSelector(
      (state: RootState) => state.schedule.scheduleData
    );
    if (scheduleData) {
